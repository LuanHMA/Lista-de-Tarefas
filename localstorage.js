...
function mostrarLista(){//Mostra oque foi digitado
    ...
    for(let i = 1; i<= itemLS; i++){
        ...
        const icon = document.createElement('i');//Cria o icone;

        icon.className = 'bi bi-x-square-fill excluir';//Adiciona a classe de estilização do icone;
        icon.setAttribute('onclick',`excluir(${i})`);//Atribui o onclick para quando precisar excluir;

        li.appendChild(textoLi);//Adiciona o texto na linha;
        li.appendChild(icon)//Adiciona o icone na linha;
